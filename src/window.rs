        ...
        Some(s) if s.last_poll_ok => {
            ...
            if s.show_cursor {
                icons.push(tray_icon::TrayIconData {
                    ...
                    percent: Some(s.cursor_weekly_percent),
                    tooltip: format!(
                        "{}: {}",
...
fn refresh_usage_texts(state: &mut AppState) {
    ...
    if let Some(cursor) = data.cursor.as_ref() {
        // A present `data.cursor` now always carries real usage (the poller errors
        // out rather than returning a default 0%), so format the value directly —
        // a genuine 0% shows "0%", never a blank "--".
...
fn codex_accent_color(_is_dark: bool) -> Color {
    Color::from_hex("#10A37F")
}
...
fn codex_usage_text_color(is_dark: bool) -> Color {
    if is_dark {
        Color::from_hex("#26C4A0")
    } else {
        Color::from_hex("#0E8A6B")
    }
}
...
fn paint_content(
    ...
    unsafe {
        ...
        let visible_windowed =
            show_claude_code as i32 + show_codex as i32 + show_antigravity as i32;
        let active =
            active_model_count(show_claude_code, show_codex, show_antigravity, show_cursor);
        let segment_count = row_bar_segment_count(active);
        let cursor_x = content_x
            + sc(LABEL_WIDTH)
            + sc(LABEL_RIGHT_MARGIN)
            + visible_windowed * (model_usage_width(segment_count) + sc(MODEL_RIGHT_MARGIN));
        let cursor_center_y = (row1_y + row2_y) / 2;
        let cursor_value_color = if active > 1 {
...
fn do_poll(send_hwnd: SendHwnd) {
    ...
    match poller::poll_with_cursor(show_claude_code, show_codex, show_antigravity, show_cursor) {
        Ok(data) => {
            let mut state = lock_state();
            if let Some(s) = state.as_mut() {
                if let Some(claude_code) = data.claude_code.as_ref() {
        ...
        Err(failure) => {
            let auth_watch = failure
                .credential_watch_mode
