...
fn main() {
    ...
    if let Some(index) = args.iter().position(|arg| arg == "--render-app-icon-weights") {
        let dir = std::path::PathBuf::from(args.get(index + 1).map(String::as_str).unwrap_or("app-icon-weights"));
        match tray_paint::write_app_icon_weights(&dir) {
            Ok(count) => diagnose::log(format!("wrote {count} weight variants to {}", dir.display())),
            Err(error) => diagnose::log(format!("weight variants failed: {error}")),
        }
        return;
    }
