...
## Validation (Codex gpt-5.6-sol high, read-only; validator ≠ implementer)
6 blockers confirmed fixed, 7 partial; regressions found and fixed in the follow-up commit: Cursor temp-copy sweep now runs after the instance mutex; settings writes serialized with unique temp names and never saved over an unreadable file (tray + panel); WSL stdout reader bounded to 4 MB; checksum body capped; provider toggle-on goes through the retry cooldown; a poll tick during a running round is dropped (no continuous polling when rounds outlast the interval); pre-poll credential snapshots are reused for the next watch; quarantine re-reads before renaming; native watch signatures carry ms mtimes; OpenCode watches WSL, Devin watches .env.devin, Fireworks watches mtime; panel cooldown mirrors the tray's and hides Retry for disabled providers. Still open (debt): OpenCode reads stop at the first parse-valid native source; schema_version is written, not yet validated; the window thread still does small local file writes (activity log, settings).

## Debt wave (same day) — plan tmp/conductor/plans/credential-source-debt.md
The four deferred items landed: one credential engine (`src/poller/credentials.rs`) behind every provider; OpenCode falls through a rejected native cookie; schema versions acted on per file with downgrade-safe rules; settings as a provider map with permanent `show_*` mirrors. Validated by Grok 4.6 + Codex gpt-5.6-sol on the same diff; their findings fixed before the final commit. 147 tests.

## Defaults / status / locations wave (2026-08-28 late) — validation
Codex gpt-5.6-sol and Grok 4.6 on the same diff (3d3fac2..afc02ce): defaults, the single HTTP client with its failure taxonomy, the trail-driven reports and the root-default home scan confirmed; both found the same gaps, all fixed before the final commit: user WSL paths reached `sh -lc` unquoted (now one shell word with only a leading `~/`/`$HOME/` allowed to expand; a `$` elsewhere no longer disables quoting); present-but-unreadable files and failed wsl.exe calls were "not installed" (now `Unreadable` with the reason); a WSL timeout's card still said "not installed" (now says WSL did not answer); the panel ignored failure-only cache changes; the WSL-user and extra-path editors only persisted on blur (now committed as typed, merged on close); Settings enumerated WSL on the UI thread (now a thread); the folded not-installed card had no Retry; pre-first-poll providers were folded as not installed; a timed-out home scan was cached for ten minutes. Accepted as-is: the schema-3 migration drops v2 provider switches (v2 never left this machine).
