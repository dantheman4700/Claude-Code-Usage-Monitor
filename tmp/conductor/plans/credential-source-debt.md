...
OUTCOME — 2026-08-28 (waves 0a54111 → 2f61431 → b067165 → final)
- Built as planned on the host lane; `bash scripts/gate.sh` green after every wave (126 → 134 → 136 → 143 → 147 tests). Providers: claude 974→847, codex 888→785, antigravity 628→591, opencode 460→365, cursor 438→406, grok 382→332, fireworks 354→319, devin 314→278 lines; `credentials.rs` ~470 lines including tests.
- Validation (both strong validators, same diff + claims): Grok 4.6 (16/20) and Codex gpt-5.6-sol converged on three defects — the panel's ordinary save wrote settings loaded leniently (could overwrite a newer file); the engine's fixed order moved Cursor's desktop store behind the CLI file; a later/post-refresh transient hid an earlier rejection. Codex added: post-refresh retry errors were discarded (a retry AuthRequired could not upgrade a TokenExpired; a retry transient did not stop the loop); a corrupt ≤v2 settings file quarantined to defaults and the next poll pruned history to 14 days; Devin allowance precedence had flipped. All fixed: panel refuses to save settings it did not really load; `NativeExtra.before_files` restores Cursor's env → desktop DB → CLI file → WSL; retry answers count (rejection outranks, transient stops); history retention is read only from a settings file that decoded as written; Devin's environment allowance wins again; per-provider sign-in hints restored; quarantine aside names carry the version; the frozen LegacySettingsV1 now has every v1 field. Host arbitration: no disagreements between the validators, only different coverage.
- Live: deployed twice; the same five signed-in providers report; settings.json untouched until a natural save, then schema 2 with the providers map and the user's choices preserved.
- Known limits carried forward: OpenCode/Fireworks/Devin paths doc-grounded; `${VAR:-…}` path expressions containing spaces would word-split in the WSL scripts (pre-existing; the transport forbids quoting); Claude/Antigravity now try WSL after a native 401 (intended: a stale native token must not hide a WSL login) at the cost of up to one wsl.exe probe per distro in that case.
